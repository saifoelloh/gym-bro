    ...
    <main className="max-w-2xl mx-auto p-4 space-y-6">
      ...
      <Card className="flex flex-col md:flex-row gap-5 items-stretch md:items-center p-5">
        <div className="flex-1">
          <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 block ml-1">Exercise</label>
          <select value={exerciseId ?? ''} onChange={e => setExerciseId(e.target.value || undefined)}
            className="w-full rounded-xl bg-surface border border-border px-4 py-3 text-sm text-white focus:border-blue-500 outline-none transition-all shadow-inner">
            <option value="">All Exercises</option>
            {exercises.map(ex => <option key={ex.id} value={ex.id}>{ex.name}</option>)}
          </select>
        </div>
        <div className="md:w-auto">
          <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 block ml-1">Time Range</label>
          <div className="flex gap-1 bg-surface p-1 rounded-xl border border-border">
            {RANGES.map(r => (
              <button key={r} onClick={() => setRange(r)}
                className={`flex-1 md:flex-none rounded-lg px-5 py-2 text-xs font-bold transition-all ${range === r ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-gray-500 hover:text-gray-300'}`}>
                {r === 7 ? '7D' : r === 30 ? '30D' : '90D'}
              </button>
            ))}
          </div>
