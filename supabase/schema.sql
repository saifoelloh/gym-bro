...
ALTER TABLE workout_templates ADD COLUMN user_id UUID REFERENCES auth.users(id) DEFAULT auth.uid();
ALTER TABLE template_exercises ADD COLUMN user_id UUID REFERENCES auth.users(id) DEFAULT auth.uid();
...
CREATE POLICY "Users can manage their own workout_templates" ON workout_templates FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can manage their own template_exercises" ON template_exercises FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
